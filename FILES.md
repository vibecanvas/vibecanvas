...
| ❓ | `components/file/viewers/ImageViewer.css` |  | Centered contained image viewer with loading overlay and muted backdrop |
| ❓ | `components/file/viewers/ImageViewer.tsx` |  | Renders image previews with loading, error, deleted states |
...
| ❓ | `plugins/camera-control/tx.sync-hand-layer.ts` |  | Hand-overlay visibility, hit-testing, cursor synchronization during panning |
...
| ❓ | `plugins/group/tx.sync-draggability.ts` |  | Group nesting disables children dragging; selected nodes re-enable dragging |
...
| ❓ | `services/crdt/CrdtService.ts` |  | Canvas CRDT service with tracked local writes |
