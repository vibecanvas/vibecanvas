...
/**
 * Apply clone action to an ellipse element.
 ...
 */
export function applyClone(ctx: TApplyContextRadius, action: TCloneAction): TChanges {
  const ellipse: TBackendElementOf<'ellipse'> = {
    ...structuredClone(ctx.element),
    id: action.id ?? crypto.randomUUID(),
    parentGroupId: action.parent?.id ?? null,
  } as TBackendElementOf<'ellipse'>
  ...
  return {
    ...
    timestamp: Date.now(),
    changes: [
      { op: 'insert', dest: 'crdt', path: ['elements', ellipse.id], value: ellipse }
    ]
  }
}
