...
import { applyDelete } from "./file.apply-delete";
...
function hasBoundsChanged(next: TFileBounds, prev: TFileBounds | null): boolean {
  if (!prev) return true;
  return (
...
const FILE_SUPPORTED_ACTIONS: ReadonlySet<TActionType> = new Set([
  "setPosition",
  "move",
  "rotate",
  "scale",
...
export class FileElement extends AElement<"file"> {
  private graphics: Graphics = new Graphics();
  private overlayDiv: HTMLDivElement | null = null;
  private setBounds: ((bounds: TFileBounds) => void) | null = null;
  private cleanupViewportSync: (() => void) | null = null;
  private lastBounds: TFileBounds | null = null;

  constructor(element: TBackendElementOf<"file">, canvas: Canvas) {
    super(element, canvas);
    this.container.addChild(this.graphics);
    this.container.label = "file-drawing-renderable-container";
    this.redraw();
    ...
    this.setupPointerListeners("file", commands);

    void this.renderFileComponent();
  ...
  private async renderFileComponent() {
    ...
    const [bounds, setBounds] = createSignal<TFileBounds>(initialBounds);
    ...
    const fileModule = await import("@/features/file-widget/components/file-canvas-widget");
    const FileCanvasWidget = fileModule.FileCanvasWidget;

    render(() => createComponent(FileCanvasWidget, {
      elementId: this.element.id,
      path: this.element.data.path,
      renderer: this.element.data.renderer,
      bounds,
  ...
  private getScreenBounds(): TFileBounds {
  ...
  static isFileElement(instance: AElement): instance is FileElement {
    return instance instanceof FileElement;
  }
}
