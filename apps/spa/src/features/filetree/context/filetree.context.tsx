...
export function createFiletreeContextLogic(args: TCreateFiletreeContextLogicArgs) {
  ...
  const handleNodeDragStart = (node: TTreeNode, event: DragEvent) => {
    const payload = {
      path: node.path,
      name: node.name,
      is_dir: node.is_dir,
  ...
  const handleNodeDragEnter = (node: TTreeNode, parentPath: string, event: DragEvent) => {
  ...
  const handleNodeDragOver = (node: TTreeNode, parentPath: string, event: DragEvent) => {
    if (!hasMoveDragPayload(event) && !draggedNode()) return;
    const currentDraggedNode = draggedNode();
    if (!currentDraggedNode) return;
    const destinationPath = node.is_dir ? node.path : parentPath;
  ...
  const handleNodeDrop = (node: TTreeNode, parentPath: string, event: DragEvent) => {
    clearFolderAutoOpenTimer();
    setDragOverTargetPath(null);
    setIsRootDropTarget(false);
  ...
  const handleNodeClick = (node: TTreeNode) => {
