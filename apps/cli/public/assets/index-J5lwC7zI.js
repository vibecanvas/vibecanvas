import { P as L, F as E, D as $, a3 as j, E as y, G as S, I as G, __tla as __tla_0 } from "./index-Bd9yMaDE.js";
let _, Z, w, W;
let __tla = Promise.all([
    ...
    class v {
        ...
        storeNode(t, e, s, i = 4, h = !1) {
            ...
            if (!h || this.pos == s) this.buffer.push(t, e, s, i);
                ...
                if (r > 0 && (this.buffer[r - 4] != 0 || this.buffer[r - 1] < 0)) {
                    let n = !1;
                    for(let o = r; o > 0 && this.buffer[o - 2] > s; o -= 4)if (this.buffer[o - 1] >= 0) {
                        n = !0;
                        break;
                    }
                    if (n) for(; r > 0 && this.buffer[r - 2] > s;)this.buffer[r] = this.buffer[r - 4], this.buffer[r + 1] = this.buffer[r - 3], this.buffer[r + 2] = this.buffer[r - 2], this.buffer[r + 3] = this.buffer[r - 1], r -= 4, i > 4 && (i -= 4);
                }
        ...
        recoverByInsert(t) {
            ...
            if (e.length > 8 || this.stack.length >= 120) {
                ...
                if (this.stack.length < 120) for(let h = 0; i.length < 8 && h < e.length; h += 2){
                    let r = e[h + 1];
        ...
        findForcedReduction() {
            let { parser: t } = this.p, e = [], s = (i, h)=>{
                if (!e.includes(i)) return e.push(i), t.allActions(i, (r)=>{
    ...
    function O(l, t, e, s, i, h) {
        ...
        t: for(; (n & l[r]) != 0;){
            let a = l[r + 1];
            for(let c = r + 3; c < a; c += 2)if ((l[c + 1] & n) > 0) {
                let d = l[c];
            ...
            for(; f < p;){
                let c = f + p >> 1, d = a + c + (c << 1), b = l[d], M = l[d + 1] || 65536;
                if (u < b) p = c;
                else if (u >= M) f = c + 1;
                else {
                    r = l[d + 2], t.advance();
    ...
    function D(l, t, e) {
        ...
        for(s.moveTo(t);;)if (!(e < 0 ? s.childBefore(t) : s.childAfter(t))) for(;;){
            if ((e < 0 ? s.to < t : s.from > t) && !s.type.isError) return e < 0 ? Math.max(0, Math.min(s.to - 1, t - 25)) : Math.min(l.length, Math.max(s.from + 1, t + 25));
    ...
    class J {
        ...
        nextFragment() {
            ...
            if (t) {
                for(this.safeFrom = t.openStart ? D(t.tree, t.from + t.offset, 1) - t.offset : t.from, this.safeTo = t.openEnd ? D(t.tree, t.to + t.offset, -1) - t.offset : t.to; this.trees.length;)this.trees.pop(), this.start.pop(), this.index.pop();
    ...
    class K {
        ...
        getActions(t) {
            let e = 0, s = null, { parser: i } = t.p, { tokenizers: h } = i, r = i.stateSlot(t.state, 3), n = t.curContext ? t.curContext.hash : 0, o = 0;
            for(let a = 0; a < h.length; a++){
                ...
                if (!(s && !u.fallback) && ((u.contextual || f.start != t.pos || f.mask != r || f.context != n) && (this.updateCachedToken(f, u, t), f.mask = r, f.context = n), f.lookAhead > f.end + 25 && (o = Math.max(f.lookAhead, o)), f.value != 0)) {
                    let p = e;
                    if (f.extended > -1 && (e = this.addActions(t, f.extended, f.end, e)), e = this.addActions(t, f.value, f.end, e), !u.extend && (s = f, e > p)) break;
                }
            }
        ...
        updateCachedToken(t, e, s) {
            ...
            if (e.token(this.stream.reset(i, t), s), t.value > -1) {
                let { parser: h } = s.p;
                for(let r = 0; r < h.specialized.length; r++)if (h.specialized[r] == t.value) {
        ...
        addActions(t, e, s, i) {
            ...
            for(let o = 0; o < 2; o++)for(let a = r.stateSlot(h, o ? 2 : 1);; a += 3){
                if (n[a] == 65535) if (n[a + 1] == 1) a = k(n, a + 2);
                else {
    ...
    class Q {
        ...
        advance() {
            ...
            if (!s.length) {
                let r = i && X(i);
                if (r) return g && console.log("Finish with " + this.stackID(r)), this.stackToTree(r);
                if (this.parser.strict) throw g && i && console.log("Stuck with token " + (this.tokens.mainToken ? this.parser.getName(this.tokens.mainToken.value) : "none")), new SyntaxError("No parse at " + e);
                this.recovering || (this.recovering = 5);
            }
            if (this.recovering && i) {
                let r = this.stoppedAt != null && i[0].pos > this.stoppedAt ? i[0] : this.runRecovery(i, h, s);
                if (r) return g && console.log("Force-finish " + this.stackID(r)), this.stackToTree(r.forceAll());
            }
            if (this.recovering) {
                ...
                t: for(let r = 0; r < s.length - 1; r++){
                    ...
                    for(let o = r + 1; o < s.length; o++){
                        let a = s[o];
                        if (n.sameState(a) || n.buffer.length > 500 && a.buffer.length > 500) if ((n.score - a.score || n.buffer.length - a.buffer.length) > 0) s.splice(o--, 1);
                        else {
                            s.splice(r--, 1);
                            continue t;
                        }
        ...
        advanceStack(t, e, s) {
            ...
            if (this.fragments) {
                ...
                for(let f = this.fragments.nodeAt(i); f;){
                    ...
                    if (p > -1 && f.length && (!a || (f.prop(y.contextHash) || 0) == u)) return t.useNode(f, p), g && console.log(r + this.stackID(t) + ` (via reuse of ${h.getName(f.type.id)})`), !0;
            ...
            if (n > 0) return t.reduce(n), g && console.log(r + this.stackID(t) + ` (via always-reduce ${h.getName(n & 65535)})`), !0;
            ...
            for(let a = 0; a < o.length;){
                ...
                if (d.apply(u, f, b ? b.start : d.pos, p), g && console.log(r + this.stackID(d) + ` (via ${(u & 65536) == 0 ? "shift" : `reduce of ${h.getName(u & 65535)}`} for ${h.getName(f)} @ ${i}${d == t ? "" : ", split"})`), c) return !0;
        ...
        runRecovery(t, e, s) {
            ...
            for(let r = 0; r < t.length; r++){
                ...
                if (n.deadEnd && (h || (h = !0, n.restart(), g && console.log(u + this.stackID(n) + " (restarted)"), this.advanceFully(n, s)))) continue;
                let f = n.split(), p = u;
                for(let c = 0; c < 10 && f.forceReduce() && (g && console.log(p + this.stackID(f) + " (via force-reduce)"), !this.advanceFully(f, s)); c++)g && (p = this.stackID(f) + " -> ");
                for (let c of n.recoverByInsert(o))g && console.log(u + this.stackID(c) + " (via recover-insert)"), this.advanceFully(c, s);
                this.stream.end > n.pos ? (a == n.pos && (a++, o = 0), n.recoverByDelete(o, a), g && console.log(u + this.stackID(n) + ` (via recover-delete ${this.parser.getName(o)})`), B(n, s)) : (!i || i.score < f.score) && (i = f);
            }
            return i;
        }
        stackToTree(t) {
    ...
    w = class extends L {
        constructor(t){
            if (super(), this.wrappers = [], t.version != 14) throw new RangeError(`Parser version (${t.version}) doesn't match runtime version (14)`);
            ...
            if (t.nodeProps) for (let n of t.nodeProps){
                ...
                for(let a = 1; a < n.length;){
                    ...
                    if (u >= 0) h(u, o, n[a++]);
                        ...
                        let f = n[a + -u];
                        for(let p = -u; p > 0; p--)h(n[a++], o, f);
                        a++;
                    }
                }
            }
            this.nodeSet = new E(e.map((n, o)=>$.define({
        ...
        hasAction(t, e) {
            ...
            for(let i = 0; i < 2; i++)for(let h = this.stateSlot(t, i ? 2 : 1), r;; h += 3){
                if ((r = s[h]) == 65535) if (s[h + 1] == 1) r = s[h = k(s, h + 2)];
                else {
                    if (s[h + 1] == 2) return k(s, h + 2);
                    break;
                }
                if (r == e || r == 0) return k(s, h + 1);
            }
            return 0;
        }
        stateSlot(t, e) {
        ...
        allActions(t, e) {
            let s = this.stateSlot(t, 4), i = s ? e(s) : void 0;
            for(let h = this.stateSlot(t, 1); i == null; h += 3){
                if (this.data[h] == 65535) if (this.data[h + 1] == 1) h = k(this.data, h + 2);
                else break;
                i = e(k(this.data, h + 1));
            }
            return i;
        ...
        nextStates(t) {
            ...
            for(let s = this.stateSlot(t, 1);; s += 3){
                if (this.data[s] == 65535) if (this.data[s + 1] == 1) s = k(this.data, s + 2);
                else break;
        ...
        parseDialect(t) {
            ...
            let i = null;
            for(let h = 0; h < e.length; h++)if (!s[h]) for(let r = this.dialects[e[h]], n; (n = this.data[r++]) != 65535;)(i || (i = new Uint8Array(this.maxTerm + 1)))[n] = 1;
            return new V(t, s, i);
        }
...
export { _ as C, Z as E, w as L, W as a, __tla };
