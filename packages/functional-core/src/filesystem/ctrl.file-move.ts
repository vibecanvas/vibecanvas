...
export function ctrlFileMove(portal: TPortal, args: TArgs): TErrTuple<TFileMoveResult> {
  ...
  if (!portal.fs.existsSync(sourcePath)) {
    return [null, { code: FilesystemErr.FILE_MOVE_SOURCE_NOT_FOUND, statusCode: 404, externalMessage: { en: "Source path not found" } }];
  }

  if (!portal.fs.existsSync(destinationDirPath)) {
    return [null, { code: FilesystemErr.FILE_MOVE_DESTINATION_NOT_FOUND, statusCode: 404, externalMessage: { en: "Destination folder not found" } }];
  }
  ...
  try {
    ...
    return [null, { code: FilesystemErr.FILE_MOVE_STAT_FAILED, statusCode: 403, externalMessage: { en: "Cannot access source or destination" } }];
  }

  if (!destinationStats.isDirectory()) {
    return [null, { code: FilesystemErr.FILE_MOVE_DESTINATION_NOT_DIRECTORY, statusCode: 400, externalMessage: { en: "Destination must be a folder" } }];
  }
  ...
  if (sourceStats.isDirectory() && (destinationResolved === sourceResolved || destinationResolved.startsWith(`${sourceResolved}${portal.path.sep}`))) {
    return [null, { code: FilesystemErr.FILE_MOVE_CANNOT_MOVE_INTO_DESCENDANT, statusCode: 400, externalMessage: { en: "Cannot move a folder into itself or its subfolder" } }];
  }

  if (portal.fs.existsSync(targetPath)) {
    return [null, { code: FilesystemErr.FILE_MOVE_TARGET_EXISTS, statusCode: 409, externalMessage: { en: "A file or folder with the same name already exists in destination" } }];
  }

  try {
    portal.fs.renameSync(sourcePath, targetPath);
  } catch {
    return [null, { code: FilesystemErr.FILE_MOVE_FAILED, statusCode: 500, externalMessage: { en: "Failed to move file or folder" } }];
  }
