...
describe("opencode-pty service helpers", () => {
  ...
  test("createOpencodePtyService forwards through injected safe client", async () => {
    ...
    expect(safeClient.api.opencode.pty.list).toHaveBeenCalledWith({ workingDirectory: "/tmp/demo" });
    expect(safeClient.api.opencode.pty.create).toHaveBeenCalledWith({ workingDirectory: "/tmp/demo", body: { title: "Terminal" } });
    expect(safeClient.api.opencode.pty.get).toHaveBeenCalledWith({ workingDirectory: "/tmp/demo", path: { ptyID: "pty-1" } });
    expect(safeClient.api.opencode.pty.update).toHaveBeenCalledWith({
      workingDirectory: "/tmp/demo",
      path: { ptyID: "pty-1" },
      body: { size: { rows: 24, cols: 80 } },
    });
    expect(safeClient.api.opencode.pty.remove).toHaveBeenCalledWith({ workingDirectory: "/tmp/demo", path: { ptyID: "pty-1" } });
  });
});
