...
import type { THandleDragSnapshot, TPoint, TShape1dData, TShape1dNode } from "./Shape1d.shared";
import { CURVE_TENSION, getElementData } from "./Shape1d.shared";
...
function getCurveControlPoints(points: TPoint[], index: number) {
  ...
  const cp1: TPoint = [p1[0] + ((p2[0] - p0[0]) / 6) * CURVE_TENSION, p1[1] + ((p2[1] - p0[1]) / 6) * CURVE_TENSION];
  const cp2: TPoint = [p2[0] - ((p3[0] - p1[0]) / 6) * CURVE_TENSION, p2[1] - ((p3[1] - p1[1]) / 6) * CURVE_TENSION];
  return { p1, cp1, cp2, p2 };
}

function evaluateCubicPoint(p1: TPoint, cp1: TPoint, cp2: TPoint, p2: TPoint, t: number): TPoint {
  const mt = 1 - t;
  return [mt ** 3 * p1[0] + 3 * mt ** 2 * t * cp1[0] + 3 * mt * t ** 2 * cp2[0] + t ** 3 * p2[0], mt ** 3 * p1[1] + 3 * mt ** 2 * t * cp1[1] + 3 * mt * t ** 2 * cp2[1] + t ** 3 * p2[1]];
}
...
export function applyAnchorDrag(node: TShape1dNode, drag: THandleDragSnapshot, worldPoint: { x: number; y: number }) {
