import type { TArrowData, TElement } from "@vibecanvas/automerge-service/types/canvas-doc";
...
function getSelfRect(node: TShape1dNode) {
  const data = node.getAttr("vcElementData") as TShape1dData | undefined;
  const strokeWidth = getStrokeWidthFromStyle((node.getAttr("vcElementStyle") as import("@vibecanvas/automerge-service/types/canvas-doc").TElementStyle | undefined) ?? {});
