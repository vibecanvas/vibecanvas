...
class OpencodePtyService {
  ...
  async list(workingDirectory: string) {
    return this.#safeClient.api.opencode.pty.list({ workingDirectory });
  }

  async create(workingDirectory: string, body?: TPtyCreateBody) {
    return this.#safeClient.api.opencode.pty.create({ workingDirectory, body });
  }

  async get(workingDirectory: string, ptyID: string) {
    return this.#safeClient.api.opencode.pty.get({
      workingDirectory,
      path: { ptyID },
    });
  }
  ...
  async update(workingDirectory: string, ptyID: string, body: TPtyUpdateBody) {
    return this.#safeClient.api.opencode.pty.update({
      workingDirectory,
      path: { ptyID },
      body,
    });
  ...
  async remove(workingDirectory: string, ptyID: string) {
    return this.#safeClient.api.opencode.pty.remove({
      workingDirectory,
      path: { ptyID },
    });
  }
...
function createOpencodePtyService(safeClient: TTerminalSafeClient) {
  return new OpencodePtyService(safeClient);
}
