...
export class BunSqliteStorageAdapter implements StorageAdapterInterface {
  ...
  constructor(database: Database | string, options?: Options) {
    ...
    this.load_stmt = this.db.prepare(`SELECT data FROM ${tableName} WHERE key = :key;`);
    ...
    `);
    this.remove_stmt = this.db.prepare(`DELETE FROM ${tableName} WHERE key = :key;`);
    this.load_range_stmt = this.db.prepare(`SELECT key, data FROM ${tableName} WHERE key GLOB :prefix;`);
    this.remove_range_stmt = this.db.prepare(`DELETE FROM ${tableName} WHERE key GLOB :prefix;`);
  }
