...
describe('executeCanvasList', () => {
  test('returns deterministic canvas inventory ordering', async () => {
    ...
    expect(renderCanvasListText(result)).toContain('Canvas inventory: 3 canvases in /tmp/vibecanvas.sqlite');
  });
  ...
  test('renders empty inventory cleanly', async () => {
    ...
    expect(renderCanvasListText(result)).toBe('Canvas inventory: 0 canvases in /tmp/test.sqlite\n');
  });
});
