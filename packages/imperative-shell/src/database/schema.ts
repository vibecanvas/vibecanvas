...
export const filetrees = sqliteTable("filetrees", {
  ...
  is_collapsed: integer("is_collapsed", { mode: "boolean" }).notNull().default(false),
  angle: real("angle").notNull().default(0),
  style: text("style", { mode: "json" }).$type<Record<string, string | number | boolean | null | string[] | number[] | boolean[]>>().notNull().default(sql`'{}'`),
  group_ids: text("group_ids", { mode: "json" }).$type<string[]>().notNull().default(sql`'[]'`),
  bound_ids: text("bound_ids", { mode: "json" }).$type<string[]>().notNull().default(sql`'[]'`),
  version: integer("version").notNull().default(1),
  version_nonce: integer("version_nonce").notNull().default(0),
